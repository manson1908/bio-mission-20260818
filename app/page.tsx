...
export default function Home() {
  ...
  return <main className="site-shell">
    <header className="topbar"><div className="brand"><img src="/logo.png" alt="新趨勢文理補習班標誌"/><div><span>新趨勢文理補習班</span><strong>國一生物互動課堂</strong></div></div><div className="top-actions">{group&&<span className="cloud-chip">● 已同步 · {group.code}</span>}<button className="teacher-button" onClick={()=>setTeacher(true)}>教師即時台</button></div></header>
    {!started?<section className="hero"><div className="hero-copy"><div className="eyebrow"><i/> 國一自然 · 生物</div><h1>一起進入<br/><em>生命的世界</em></h1><p>建立小隊後開始今天的課堂活動。作答、實作紀錄與老師回饋都會自動保存，下次可輸入小隊代碼繼續。</p><div className="join-card"><label>小隊名稱</label><input value={team} maxLength={20} onChange={e=>setTeam(e.target.value)}/><label>已有小隊代碼（選填）</label><input value={joinCode} maxLength={8} placeholder="例如 A7K2P" onChange={e=>setJoinCode(e.target.value.toUpperCase())}/><button onClick={join} disabled={busy}>{busy?"連線中…":"開始上課 →"}</button>{notice&&<small>{notice}</small>}</div></div><aside className="class-summary"><span>本單元學習重點</span><h2>生命世界與<br/>生物體的組成</h2><ul><li>判斷生命現象與環境適應</li><li>練習科學方法與變因判斷</li><li>正確操作複式顯微鏡</li><li>比較動物與植物細胞</li></ul><div><strong>{leaderboard.length}</strong><small>組已加入本課堂</small></div></aside></section>:<>
      <section className="mission-head"><div><div className="eyebrow"><i/> 小隊代碼 {group?.code}</div><h1>{team}，<em>開始上課</em></h1><p>每個單元包含四題概念檢核與一項實作紀錄，完成後可送交老師評分。</p></div><div className="hud"><div><small>目前總分</small><strong>{score.toLocaleString()}</strong></div><div><small>連續答對</small><strong className="combo">{combo} 題</strong></div><div><small>學習階段</small><strong>{rank}</strong></div></div></section>
      <section className="live-board"><div><div className="progress-label"><span>課程完成度</span><strong>{progress}%</strong></div><div className="progress-track"><span style={{width:`${progress}%`}}/></div></div><div className="mini-ranking"><strong>即時排行</strong>{leaderboard.slice(0,5).map((g,i)=><span key={g.id}><b>{i+1}</b>{g.name}<em>{g.score}</em></span>)}</div></section>
      <section className="mission-grid">{missions.map((m,i)=><button key={m.id} className={`mission-card ${m.color} ${completed.includes(m.id)?"done":""}`} onClick={()=>openMission(m)}><span className="card-number">{String(i+1).padStart(2,"0")}</span><span className="mission-icon">{m.icon}</span><span className="mission-code">單元 {m.code} · 4 題＋實作</span><strong>{m.title}</strong><small>{m.subtitle}</small><span className="card-status">{completed.includes(m.id)?"✓ 已送交":"查看內容 →"}</span></button>)}</section>
      <section className="specimen-section"><div className="specimen-intro"><div className="eyebrow"><i/> 課堂觀察圖鑑</div><h2>先看懂真實影像，<br/>再回到課本整理概念</h2><p>顯微鏡下的細胞不會像課本示意圖那麼整齊。觀察時先找外形、排列與染色較深的構造，再判斷細胞種類。</p></div><div className="specimen-grid"><figure><img src="/onion-epidermis.jpg" alt="顯微鏡下的洋蔥表皮細胞"/><figcaption><b>洋蔥表皮細胞</b><span>排列緊密、外形近似長方形，清楚的邊界是細胞壁。</span><a href="https://commons.wikimedia.org/wiki/File:Onion_epidermis.jpg" target="_blank" rel="noreferrer">圖片：SaraKasa／CC BY 4.0</a></figcaption></figure><figure><img src="/cheek-cell.jpg" alt="顯微鏡下的人類口腔皮膜細胞"/><figcaption><b>口腔皮膜細胞</b><span>外形較不規則，染色較深的圓點通常是細胞核。</span><a href="https://commons.wikimedia.org/wiki/File:Human_cheek_cell.jpg" target="_blank" rel="noreferrer">圖片：MaryViennePascual／Wikimedia Commons</a></figcaption></figure><article className="compare-card"><span>觀察提示</span><h3>怎麼分辨？</h3><dl><div><dt>細胞壁</dt><dd>植物細胞有；動物細胞沒有</dd></div><div><dt>葉綠體</dt><dd>只有部分綠色植物細胞可見</dd></div><div><dt>細胞核</dt><dd>染色後通常較容易觀察</dd></div><div><dt>共同構造</dt><dd>都有細胞膜、細胞質與細胞核</dd></div></dl></article></div></section>
    </>}
    <footer><span>© 2026 新趨勢文理補習班</span><strong>雲端課堂 · 即時回饋 · 科學思考</strong></footer>

    {active&&<div className="modal-backdrop"><section className="question-modal" role="dialog" aria-modal="true"><button className="close" onClick={()=>setActive(null)}>×</button><div className="question-top"><span>{active.icon}</span><div><small>單元 {active.code}</small><h2>{active.title}</h2></div><b>{questionIndex<active.questions.length?`${questionIndex+1}/4`:"實作"}</b></div><div className="question-progress"><span style={{width:`${Math.min(questionIndex+1,5)/5*100}%`}}/></div>
